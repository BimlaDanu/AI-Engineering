...
def test_a_covered_question_is_grounded_in_one_search() -> None:
...
def test_a_shelf_restriction_keeps_only_that_shelf() -> None:
    store = FakeStore({"": [(VQE, 0.6), (GAP_SHELVED, 0.6)]})
    found = search(store, "anything", shelves=("quantum-computing",))
    assert [passage.document for passage in found] == ["variational-quantum-eigensolver"]
...
def test_the_shelf_filter_applies_to_the_keyword_half_too() -> None:
    # A filter honoured by one half and ignored by the other is not a filter. VQE
    # is the keyword match, but it is not on the shelf the router chose.
    result = retrieve(
...
def test_the_shelf_filter_is_still_spent_after_one_round_for_the_keyword_half() -> None:
    result = retrieve(
        "variational eigensolver upper bound",
        store=FakeStore(),
        lexical=keywords(VQE),
