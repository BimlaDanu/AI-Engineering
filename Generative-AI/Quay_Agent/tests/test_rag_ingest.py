...
def test_a_retrievable_ground_state_energy_is_reported_not_indexed_silently(
    corpus: Path,
) -> None:
    # The risk is specific: a figure sitting in the corpus is retrievable, and once
    # retrieved it is indistinguishable from one the solver computed and
    # cross-checked. So the checker looks for a precise decimal *next to an energy
    # word* -- which is the shape of a number that could be passed off as this
    # project's own result.
    (corpus / "physics-notes" / "pfeuty-exact-solution.md").write_text(
        NOTE.replace(
            "closes linearly",
            "gives a ground-state energy of -1.27324 per site",
        ),
        encoding="utf-8",
    )
...
def test_an_ordinary_number_in_an_abstract_is_not_reported(corpus: Path) -> None:
    # This is the gate the checker's docstring promised and never had, and it exists
    # because the pattern used to be `\d\.\d{3,}` -- any three-place decimal. That
    # fired on 90 of the real corpus's 127 documents, because a corpus of
    # quantum-computing abstracts is made of numbers: gate fidelities, error rates,
    # exponents. Ninety identical warnings on a successful ingest is not a control,
    # and this repository has a rule about it: a checker that cries wolf gets
    # ignored on the run where it is right.
    (corpus / "physics-notes" / "pfeuty-exact-solution.md").write_text(
        NOTE.replace(
            "closes linearly",
            "was demonstrated with a two-qubit gate fidelity of 99.937 per cent",
        ),
        encoding="utf-8",
    )
...
def test_a_headings_only_chunk_is_never_indexed(corpus: Path) -> None:
    # The bug this closes was visible in a live log and easy to miss: 32 of 463
    # chunks carried no prose, so `retrieve` rejected them -- correctly -- on every
    # single search. The rule existed at query time and not at ingest time, so the
    # index stored 7% of itself as material that could never be cited, each rejected
    # chunk took a slot in the candidate pool a real passage could have used, and
    # with query expansion running four phrasings per question it produced about
    # thirty rejection lines for one answer. A log nobody can read is a log nobody
    # reads.
    (corpus / "physics-notes" / "headings.md").write_text(
        NOTE.split("The chain is one")[0] + "\n## Another heading\n\n## And another\n",
        encoding="utf-8",
    )
    report = ingest(FakeIndex(), root=corpus)
    assert report.skipped, "a headings-only chunk should be reported as skipped"
    assert all("headings" in identifier for identifier in report.skipped), report.skipped


def test_the_two_prose_rules_cannot_drift_apart() -> None:
    # `ingest.carries_prose` and `retrieve.has_prose` are deliberately separate --
    # ingest must not import the retrieval layer, because ingest runs before anything
    # searches. Separate definitions of one rule is exactly how a chunk gets written
    # by one and refused by the other, which is the defect above. So they are held
    # together here instead.
    from src.rag.ingest import carries_prose
    from src.rag.retrieve import has_prose
    ...
    for text in cases:
        assert carries_prose(text) == has_prose(text), repr(text)
