...
:mod:`src.physics.ed`'s basis, because the two would then share a convention and a
mistake in it would cancel. :attr:`Quantumness.energy_disagreement` is this
module's ground-state energy against the free-fermion closed form -- the same
cross-check every other number in the project goes through, applied to the
matrices these identities are evaluated on.
...
def terms(spec: TFIMSpec) -> tuple[ComplexArray, ComplexArray]:
    ...
    for left, right in ed.bonds(spec):
        ising -= spec.coupling * (
...
def measure(spec: TFIMSpec) -> Quantumness:
    ...
    reference = exact.ground_state_energy(spec) if exact.unsupported_reason(spec) is None else None
