...
module runs what the grader's bench says is applicable and reports what came
back;
...
from src.physics.method_catalogue import Rejection
...
class MethodResult:
    ...
    Attributes:
        method: The catalogue name of the method that produced the number.
...
class CrossCheck:
    ...
    def summary(self) -> str:
        ...
        spread = self.max_disagreement
        if not self.results:
            # Distinct from the one-method case below. Both leave the spread
            # undefined, and reporting them the same way had the report say "no
            # method could solve this" and "only one method applies" in
            # consecutive lines.
            lines.append("  UNCORROBORATED: no method applies, so there is nothing to check")
        elif spread is None:
...
def cross_check(spec: TFIMSpec) -> CrossCheck:
    ...
    Examples:
        ...
        False
    """
    available = exact_survey(spec)
    results = tuple(
        MethodResult(method=facts.name, energy=facts.solve(spec)) for facts in available.applicable
    )
