...
The first layer of an OWASP LLM01 defence and the only layer allowed to block.
Pure Python: no model, no key, no network, and no judgement that cannot be read
off the source. A guardrail implemented by a language model is one an attacker
can argue with, in the same channel as the attack.

One signal blocks. There is no score to tune, because the two errors are not
symmetric: a false positive costs one rephrase, a false negative hands the
prompt to an attacker who then speaks with the agent's voice. The domain is one
Hamiltonian, so nobody asking about the transverse-field Ising chain needs the
...
collapses whitespace first. Those characters are a signal in their own right: a
physics question does not contain them by accident.

Both directions are screened. A typed question is the obvious surface; the
subtler one is retrieved text, since a document saying *disregard your
instructions* is an injection nobody typed. :func:`screen` takes any string and
:func:`neutralise` defangs text that must be included rather than rejected.

The layered half lives in :mod:`src.agent.router`, which consults a model
classifier only on text this module has already cleared -- an ordering that lets
the classifier add a block and never remove one.
"""
...
a paste accident or an attempt to hide an instruction deep enough that attention
...
class Screening:
    ...
    def explain(self) -> str:
        ...
        lines = [f"blocked: {count} signal{'' if count == 1 else 's'} detected"]
        lines.extend(
